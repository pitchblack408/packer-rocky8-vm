...
ESXI_CUSTOMIZER_HOME="/opt/esxi-customizer"
mkdir -p $ESXI_CUSTOMIZER_HOME

cat << 'EOF' > "$ESXI_CUSTOMIZER_HOME/ESXi-Customizer-PS.ps1"
...
chmod +x "$ESXI_CUSTOMIZER_HOME/ESXi-Customizer-PS.ps1"

#####################################################################################################################


cat << EOF > "$ESXI_CUSTOMIZER_HOME/esxi-customizer"
#!/bin/bash

# Set the ESXI_CUSTOMIZER_HOME variable to the actual value
ESXI_CUSTOMIZER_HOME="$ESXI_CUSTOMIZER_HOME"

# Extract the PowerCLI script path
ARGS="\$1"

if [ -n "\$ARGS" ]; then 
   docker run --rm --entrypoint="/usr/bin/pwsh" -v "\$ESXI_CUSTOMIZER_HOME:/tmp/scripts" vmware/powerclicore /tmp/scripts/ESXi-Customizer-PS.ps1 "\$ARGS"
else
   docker run --rm --entrypoint="/usr/bin/pwsh" -v "\$ESXI_CUSTOMIZER_HOME:/tmp/scripts" vmware/powerclicore /tmp/scripts/ESXi-Customizer-PS.ps1
fi
EOF

chmod +x "$ESXI_CUSTOMIZER_HOME/esxi-customizer"
ln -s "$ESXI_CUSTOMIZER_HOME/esxi-customizer" "/usr/local/bin/esxi-customizer"
